...
// ==========================================
// INTELIGÊNCIA ARTIFICIAL E MINIMAX
// ==========================================
function evaluateBoard(board) {
    let totalEvaluation = 0;
    const pieceValues = { '♙': -10, '♖': -50, '♘': -30, '♗': -30, '♕': -90, '♔': -900,
                          '♟': 10, '♜': 50, '♞': 30, '♝': 30, '♛': 90, '♚': 900 };
    for (let r = 0; r < 8; r++) {
        for (let c = 0; c < 8; c++) {
            if (board[r][c]) {
                totalEvaluation += pieceValues[board[r][c]] || 0;
                if (r >= 3 && r <= 4 && c >= 3 && c <= 4) {
                    totalEvaluation += getPieceColor(board[r][c]) === 'black' ? 1 : -1;
                }
            }
        }
    }
...
function minimax(board, depth, alpha, beta, isMaximizingPlayer) {
    ...
    if (possibleMoves.length === 0) return isKingInCheck(color, board) ? (isMaximizingPlayer ? -9999 : 9999) : 0;
...
function playAI() {
    ...
    if (mode === 'ai-easy' || mode === 'ai-medium' || mode === 'ai-hard') {
        ...
        for (let m of moves) {
            ...
            if (mode === 'ai-medium' && target) {
                score += pieceValues[target] || 0;
            } 
            else if (mode === 'ai-hard') {
                if (target) score += (pieceValues[target] || 0) * 10;
                
                let simBoard = boardLayout.map(row => [...row]);
                simBoard[m.tR][m.tC] = m.piece;
                simBoard[m.fR][m.fC] = '';

                let isAttacked = false;
                for(let r=0; r<8; r++) for(let c=0; c<8; c++) 
                    if(getPieceColor(simBoard[r][c]) === 'white' && isPseudoLegalMove(r, c, m.tR, m.tC, simBoard[r][c], simBoard)) isAttacked = true;

                if (isAttacked) score -= (pieceValues[m.piece] || 0) * 10;
                else score += 5;
        ...
        statusElement.innerText = 'A IA está pensando... (Profundidade 3)';
